...
package com.yalin.wallpaper.yinyang;
 ...
 */
public final class YinYangService extends WallpaperServiceProxy {

    private Engine mEngine;

    public YinYangService(Context host) {
        super(host);
    }
    ...
    private final class YinYangSurfaceView extends GLSurfaceView implements
        ...
        public final void onDrawFrame(GL10 unused) {
            ...
            if (!mTouchFollow) {
                ...
                float t = Math.max(0f, 1f - (currentTime - mTouchTime) * .005f);
                ...
                if (Math.abs(mTouchPositions[0] - mTouchPositions[2]) > 0.001f
                        || Math.abs(mTouchPositions[1] - mTouchPositions[3]) > 0.001f) {
                    requestRender();
                }
            }
