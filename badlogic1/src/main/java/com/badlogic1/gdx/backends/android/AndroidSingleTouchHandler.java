...
public class AndroidSingleTouchHandler implements AndroidTouchHandler {
	public void onTouch (MotionEvent event, AndroidInput input) {
		...
		if (event.getAction() == MotionEvent.ACTION_DOWN) {
			postTouchEvent(input, AndroidInput.TouchEvent.TOUCH_DOWN, x, y, 0, timeStamp);
			input.touched[0] = true;
			input.deltaX[0] = 0;
			input.deltaY[0] = 0;
		} else if (event.getAction() == MotionEvent.ACTION_MOVE) {
			postTouchEvent(input, AndroidInput.TouchEvent.TOUCH_DRAGGED, x, y, 0, timeStamp);
			input.touched[0] = true;
			input.deltaX[0] = x - oldX;
			input.deltaY[0] = y - oldY;
		} else if (event.getAction() == MotionEvent.ACTION_UP) {
			postTouchEvent(input, AndroidInput.TouchEvent.TOUCH_UP, x, y, 0, timeStamp);
