...
import com.badlogic1.gdx.graphics.GL10;
import com.badlogic1.gdx.graphics.Mesh;
import com.badlogic1.gdx.graphics.VertexAttribute;
import com.badlogic1.gdx.graphics.VertexAttributes;
...
import com.badlogic1.gdx.utils.FloatArray;
import com.badlogic1.gdx.utils.GdxRuntimeException;
import com.badlogic1.gdx.utils.IntArray;
...
public class G3dtLoader {
	...
	private static KeyframedSubMesh readMesh (BufferedReader in, boolean flipV) throws IOException {
		...
		KeyframedSubMesh mesh = new KeyframedSubMesh(name, new Mesh(Mesh.VertexDataType.VertexArray, false, numVertices, indices.size,
	...
	private static VertexAttribute[] createVertexAttributes (boolean hasNormals, int uvs) {
		...
		int idx = 0;
		attributes[idx++] = new VertexAttribute(VertexAttributes.Usage.Position, 3, ShaderProgram.POSITION_ATTRIBUTE);
		if (hasNormals) attributes[idx++] = new VertexAttribute(VertexAttributes.Usage.Normal, 3, ShaderProgram.NORMAL_ATTRIBUTE);
		for (int i = 0; i < uvs; i++) {
			attributes[idx++] = new VertexAttribute(VertexAttributes.Usage.TextureCoordinates, 2, ShaderProgram.TEXCOORD_ATTRIBUTE + i);
		}
		return attributes;
	}
