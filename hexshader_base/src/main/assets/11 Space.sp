...
void main()
{
    vec2 uv = a_pos * vec2(0.5, 1.0) + vec2(u_offset, 0);
	...
	float freqs[4];
	//Sound
	freqs[0] = 0.5 * sin(iGlobalTime * 0.2) + 0.5;
	freqs[1] = 0.5 * cos(iGlobalTime * 0.3) + 0.5;
	freqs[2] = 0.5 * sin(iGlobalTime * 0.5) + 0.5;
	freqs[3] = 0.5 * cos(iGlobalTime * 0.7) + 0.5;
	...
	v_col = mix(freqs[3]-.3, 1., v) * vec4(1.5*freqs[2] * t * t* t , 1.2*freqs[1] * t * t, freqs[3]*t, 1.0)+c2+starcolor;
