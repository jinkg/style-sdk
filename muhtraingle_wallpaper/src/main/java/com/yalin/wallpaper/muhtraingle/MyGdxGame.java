...
public abstract class MyGdxGame extends ApplicationAdapter {
        ...
        for (int y = 0; y < grid.length; y++) {
            for (int x = 0; x < grid[0].length; x++) {
                ...
                if (GRADIENTTYPE == GRADIENTTYPE_RADIAL)
                    f = (float) (1 - Math.sqrt(Math.pow(((NX * SX) / 2) - xf, 2) + Math.pow(((NY * SY) / 2) - yf, 2)));
                else if (GRADIENTTYPE == GRADIENTTYPE_HORIZONTAL)
                    f = (1 - xf) * (width < height ? width / height : height / width);
                else if (GRADIENTTYPE == GRADIENTTYPE_VERTICAL)
                    f = (1 - yf) * (width < height ? width / height : height / width);
                else if (GRADIENTTYPE == GRADIENTTYPE_DIAGONAL1)
                    f = (((1 - xf) * (width < height ? width / height : height / width)) + ((1 - yf) * (width < height ? width / height : height / width))) / 2;
                else if (GRADIENTTYPE == GRADIENTTYPE_DIAGONAL2)
                    f = ((xf * (width < height ? width / height : height / width)) + ((1 - yf) * (width < height ? width / height : height / width))) / 2;
                else if (GRADIENTTYPE == GRADIENTTYPE_DIAGONAL3)
                    f = (((1 - xf) * (width < height ? width / height : height / width)) + (yf * (width < height ? width / height : height / width))) / 2;
                else if (GRADIENTTYPE == GRADIENTTYPE_DIAGONAL4)
                    f = ((xf * (width < height ? width / height : height / width)) + (yf * (width < height ? width / height : height / width))) / 2;
                else if (GRADIENTTYPE == GRADIENTTYPE_RANDOM)
                    f = (float) (0.3 + Math.random() * 0.7);

                if (GRADIENTSUBTLE == GRADIENTSUBTLE_VERY) f = neutrF * 0.5f + f * 0.5f;
                else if (GRADIENTSUBTLE == GRADIENTSUBTLE_SLIGHTLY) f = neutrF * 0.2f + f * 0.8f;
                else if (GRADIENTSUBTLE == GRADIENTSUBTLE_BARELY) f = neutrF * 0.1f + f * 0.9f;
                else if (GRADIENTSUBTLE == GRADIENTSUBTLE_MODERATE) f = neutrF * 0.35f + f * 0.65f;
                else if (GRADIENTSUBTLE == GRADIENTSUBTLE_NOTATALL) f = neutrF * -0.2f + f * 1.2f;
                else if (GRADIENTSUBTLE == GRADIENTSUBTLE_EXTREMELY) f = neutrF * 0.7f + f * 0.3f;
    ...
    public void render() {
        ...
        if (HUEMODE == HUEMODE_1W) day = time / 604800.0;
        else if (HUEMODE == HUEMODE_24H) day = time / 86400.0;
        else if (HUEMODE == HUEMODE_12H) day = time / 43200.0;
        else if (HUEMODE == HUEMODE_1H) day = time / 3600.0;
        else if (HUEMODE == HUEMODE_1M) day = time / 60.0;
        ...
        if (INSTABILITY != INSTABILITY_GLITCH && OUTLINE != OUTLINE_OFF) {
            ...
            if (OUTLINE == OUTLINE_WHITE)
                outline.setColor(1, 1, 1, OUTLINETHICKNESS == OUTLINETHICKNESS_HAIRLINE ? 0.03f : OUTLINETHICKNESS == OUTLINETHICKNESS_THIN ? 0.07f : OUTLINETHICKNESS == OUTLINETHICKNESS_THICK ? 0.16f : OUTLINETHICKNESS == OUTLINETHICKNESS_BOLD ? 0.5f : 0.1f);
            else if (OUTLINE == OUTLINE_BLACK)
                outline.setColor(0, 0, 0, OUTLINETHICKNESS == OUTLINETHICKNESS_HAIRLINE ? 0.02f : OUTLINETHICKNESS == OUTLINETHICKNESS_THIN ? 0.05f : OUTLINETHICKNESS == OUTLINETHICKNESS_THICK ? 0.1f : OUTLINETHICKNESS == OUTLINETHICKNESS_BOLD ? 0.5f : 0.08f);
            for (Triangle t : triangles) {
